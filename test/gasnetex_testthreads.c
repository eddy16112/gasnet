...
#define TEST_SEGZ_PER_THREAD ((uintptr_t)6*1024*1024)
...
threadmain(void *args)
{
	...
	if (!threadstress) MSG("tid=%3d, tid_peer=%3d> starting.", td->tid, td->tid_peer);

  int	tot_threads = numranks * threads_num;
  ...
  printf("tid=%3d> nb_copies %d\n", td->tid, iters);
  if (td->ltid == 0) printf("tid=%3d> took %lu us\n", td->tid, ((stop.tv_sec - start.tv_sec) * 1000000 + stop.tv_usec - start.tv_usec)/iters); 
